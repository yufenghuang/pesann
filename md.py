...
def specialrun14(params):

    # train with highT forces + lowT energies & forces
    ...
    tfLossForce = float(params['feRatio']) * tf.reduce_mean(tf.squared_difference(tfFs, tfFors))

    with tf.variable_scope("Adam", reuse=tf.AUTO_REUSE):
        tfOptimizer = tf.train.AdamOptimizer(tfLearningRate).minimize(tfLoss)

    with tf.variable_scope("AdamForce", reuse=tf.AUTO_REUSE):
        tfOptimizerForce = tf.train.AdamOptimizer(tfLearningRate).minimize(tfLossForce)
    ...
    for iEpoch in range(params["epoch"]):
        ...
        if params["mmtForces"] != "None":
            ...
            for iCase in range(nCase):
                ...
                sess.run(tfOptimizerForce, feed_dict=feedDict)
...
def specialrun15(params):
    ...
    tfLossForces = float(params['feRatio']) * tf.reduce_mean(tf.squared_difference(tfFs, tfFors))

    with tf.variable_scope("Adam", reuse=tf.AUTO_REUSE):
        tfOptimizer = tf.train.AdamOptimizer(tfLearningRate).minimize(tfLoss)

    with tf.variable_scope("AdamForces", reuse=tf.AUTO_REUSE):
        tfOptimizerForces = tf.train.AdamOptimizer(tfLearningRate).minimize(tfLossForces)
    ...
    for iEpoch in range(params["epoch"]):
        if params["mmtForces"] != "None":
            ...
            for iCase in range(nCase):
                ...
                sess.run(tfOptimizerForces, feed_dict=feedDict)
